...
function render() { const issues = data ? [...validateData(data), ...data.errors] : []; app.innerHTML = `<header><h1>Chronos Scheduler</h1><p>Excel 기반 교사·학생·학년 시간표 자동 생성 웹앱</p></header><main><section class="panel"><h2>Excel 업로드</h2><div id="drop"><input type="file" id="file" accept=".xlsx"/><p>.xlsx 파일을 드래그하거나 선택하세요.</p></div><button id="load">저장 프로젝트 불러오기</button></section>${data ? `<section class="grid">${card('학생 수', data.students.length)}${card('canonical 학년 수', data.grades.length)}${card('교사 수', data.teachers.length)}${card('과목 수', data.subjects.length)}${card('cohort 수', data.cohorts.length)}${card('CourseRequirement 수', data.requirements.length)}${card('전체 주당 교시', data.requirements.reduce((a, b) => a + b.totalPeriodsPerWeek, 0))}${card('수업 인스턴스', data.requirements.reduce((a, b) => a + b.meetingLengths.length, 0))}${card('오류', issues.filter(i => i.level === 'error').length, 'bad')}${card('경고', issues.filter(i => i.level === 'warning').length, 'warn')}</section><section class="panel"><h2>시트 및 검증</h2><p>${data.sourceSheets.join(', ')}</p><div class="issues">${issues.map(i => `<p class="${i.level}">● [${i.code}] ${i.message}</p>`).join('')}</div></section>${diagnostics()}${requirementsPreview()}${constraintEditor()}<section class="panel"><h2>시간표 생성</h2><label>최대 초 <input id="sec" type="number" value="5"></label><label>최대 노드 <input id="nodes" type="number" value="20000"></label><label>Seed <input id="seed" type="number" value="42"></label><button id="solve" ${data.requirements.length === 0 ? 'disabled' : ''}>시간표 생성</button><button id="save">프로젝트 저장</button><button id="xlsx">Excel 내보내기</button><button id="pdf">PDF/인쇄</button></section>` : ''}${result && data ? `<section class="panel"><h2>진행/결과</h2><p>노드 ${result.progress.nodes}, 백트래킹 ${result.progress.backtracks}, 배정 ${result.progress.assigned}/${result.progress.total}, 미배정 ${result.progress.unassigned}, 점수 ${result.score}, 종료 ${result.progress.reason}</p><h3>학년별 시간표</h3>${data.grades.slice(0, 6).map(g => `<details open><summary>${g.name}</summary>${tableFor(a => a.gradeIds.includes(g.id), a => `${nameOf(a.subjectId, 'subject')}<br><small>${a.teacherIds.map(t => nameOf(t, 'teacher')).join(', ')}</small>`)}</details>`).join('')}<h3>교사별 시간표</h3>${data.teachers.slice(0, 6).map(t => `<details><summary>${t.name}</summary>${tableFor(a => a.teacherIds.includes(t.id), a => `${nameOf(a.subjectId, 'subject')}<br><small>${a.cohortIds.map(c => nameOf(c, 'cohort')).join(', ')}</small>`)}</details>`).join('')}<h3>미배정/진단</h3>${result.issues.map(i => `<p class="error">${i.message}</p>`).join('')}</section>` : ''}</main>`; bind(); }
...
} }); document.querySelector('#solve')?.addEventListener('click', () => { if (!data || data.requirements.length === 0)
