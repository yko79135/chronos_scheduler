...
function bind() { document.querySelector('#file')?.addEventListener('change', async (e) => { const f = e.target.files?.[0]; if (!f)
    ...
    return; const maxSeconds = Number(document.querySelector('#sec').value), maxNodes = Number(document.querySelector('#nodes').value), seed = Number(document.querySelector('#seed').value); solveStatus = 'Solving'; solveError = ''; result = null; audit = null; render(); const jobId = String(Date.now()); activeWorker = new Worker(new URL('./scheduler/worker.js', import.meta.url), { type: 'module' }); activeWorker.onmessage = (ev) => { const msg = ev.data; if (msg.type === 'progress') {
    solveStatus = 'Solving';
} if (msg.type === 'error') {
    solveStatus = 'Failed';
    ...
    if (audit?.summary.hardFailed || audit?.summary.hardIncomplete)
        solveStatus = 'Failed';
    ...
    return; const slot = `${document.querySelector('#day').value}-${document.querySelector('#period').value}`; data.constraints.push({ id: `con_${Date.now()}`, type, targetIds: [id], value: type === 'after-school-only' ? 8 : slot, hard: true, source: 'manual' }); const r = data.requirements.find(x => x.id === id); if (r) {
    if (type === 'fixed-slot')
        r.fixedSlots = [slot];
    ...
    if (type === 'after-school-only')
        r.afterSchool = true;
} render(); }; document.querySelector('#fix')?.addEventListener('click', () => add('fixed-slot')); document.querySelector('#after')?.addEventListener('click', () => add('after-school-only')); document.querySelector('#allow')?.addEventListener('click', () => add('allowed-slots')); document.querySelector('#forbid')?.addEventListener('click', () => add('forbidden-slots')); document.querySelector('#lock')?.addEventListener('click', () => add('locked-assignment')); document.querySelector('#cancel')?.addEventListener('click', () => { activeWorker?.terminate(); activeWorker = null; solveStatus = 'Partial result'; render(); }); document.querySelector('#merge-shared')?.addEventListener('click', () => { if (!data)
