...
function bind() { document.querySelector('#file')?.addEventListener('change', async (e) => { const f = e.target.files?.[0]; if (!f)
    ...
    return; const maxSeconds = Number(document.querySelector('#sec').value), maxNodes = Number(document.querySelector('#nodes').value), seed = Number(document.querySelector('#seed').value); solveStatus = 'Solving'; solveError = ''; result = null; audit = null; render(); setTimeout(() => { try {
    if (!data)
        return;
    result = solveSchedule(data, { maxSeconds, maxNodes, seed, allowUnassigned: true, weights: { unassigned: 10000 } });
    audit = auditSchedule(data, result);
    if (audit.summary.hardFailed)
        solveStatus = 'Failed';
    ...
    solveStatus = 'Failed';
    solveError = err instanceof Error ? `${err.message}\n${err.stack ?? ''}` : String(err);
} render(); }, 0); }); document.querySelector('#save')?.addEventListener('click', () => data && saveProject(data, result ?? undefined)); document.querySelector('#load')?.addEventListener('click', () => { const p = loadProject(); if (p) {
    data = p.data;
