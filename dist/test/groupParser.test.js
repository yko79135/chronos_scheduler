...
import { parseGradeExpression } from '../scheduler/groupParser.js';
...
test('sectioned G7 expansion and exact section matching', () => { assert.deepEqual(parseGradeExpression('G7', reg), ['G7E', 'G7K']); assert.deepEqual(parseGradeExpression('G7E', reg), ['G7E']); assert.deepEqual(parseGradeExpression('G7K', reg), ['G7K']); assert.deepEqual(parseGradeExpression('G7-12', reg), ['G7E', 'G7K', 'G9', 'G12']); assert.deepEqual(parseGradeExpression('G1-3', reg), ['G1', 'G2', 'G3']); });
