...
test('fixed shared classes with same subject use distinct deterministic ids and slots', () => { const d = normalizeWorkbook(parsed({ '학년별': [['G7'], ['수업', '시간', '횟수', '교사'], ['Bible', 1, 1, 'T'], [], ['G9'], ['수업', '시간', '횟수', '교사'], ['Bible', 1, 1, 'T'], [], ['G10'], ['수업', '시간', '횟수', '교사'], ['Bible', 1, 1, 'T'], [], ['G11'], ['수업', '시간', '횟수', '교사'], ['Bible', 1, 1, 'T'], [], ['G12'], ['수업', '시간', '횟수', '교사'], ['Bible', 1, 1, 'T']], 규칙: [['고정수업', 'G7 Bible - 화요일 5교시, G9-12 Bible - 화요일 6교시']] })); const bible = d.requirements.filter(r => d.subjects.find(s => s.id === r.subjectId)?.name === 'Bible'); assert(bible.some(r => r.gradeIds.length === 1 && r.fixedSlots[0] === '화-5')); assert(bible.some(r => r.gradeIds.length === 4 && r.fixedSlots[0] === '화-6')); assert.equal(new Set(bible.map(r => r.id)).size, bible.length); assert(d.constraints.some(c => c.value.originalText === 'G7 Bible - 화요일 5교시')); assert(d.constraints.some(c => c.value.originalText === 'G9-12 Bible - 화요일 6교시')); });
