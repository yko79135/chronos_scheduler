...
function score(a, data) { let p = 0; const byKey = new Map(); for (const x of a) {
    ...
    if (v[i] - v[i - 1] > 1)
        p += 1; }); for (const x of a) {
    const req = data.requirements.find(r => r.id === x.instanceId.replace(/_\d+$/, ""));
    if (req?.preferredSlots.length) {
        const slots = slotRange(x.slot, x.length);
        const hit = slots.filter(s => req.preferredSlots.includes(s)).length;
        p += 5 * (slots.length - hit);
        p -= 2 * hit;
    }
} return -p; }
...
} const obj = objective(best, data); const mandatoryMissing = unassigned.filter(isMandatory); const reason = mandatoryMissing.length ? 'infeasible-hard-constraints' : unassigned.length === 0 ? 'complete' : limit ?? (exhausted ? 'optimal-partial' : 'partial-search-exhausted'); const dedup = new Map(); for (const i of issues)
    dedup.set(`${i.code}:${i.message}`, i); const finalIssues = [...dedup.values()]; const final = { assignments: best, unassigned, issues: finalIssues, score: obj.soft, progress: { elapsedMs: now() - start, nodes, backtracks, assigned: best.length, total: all.length, unassigned: unassigned.length, assignedPeriods: obj.periods, totalPeriods, unassignedPeriods: totalPeriods - obj.periods, bestScore: obj.soft, reason, seed: opt.seed } }; opt.onProgress?.(final.progress, final); return final; }
